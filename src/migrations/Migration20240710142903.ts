...
export class Migration20240710142903 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "app"."tweet" ("id" serial primary key, "tweet_id" varchar(255) not null, "text" varchar(255) not null, "created_at" timestamptz not null);');
    ...
    // this.addSql('drop table if exists "realtime"."messages" cascade;');
    ...
    // this.addSql('drop table if exists "auth"."one_time_tokens" cascade;');
  ...
  async down(): Promise<void> {
    ...
    this.addSql('create table "realtime"."messages" ("id" bigserial primary key, "topic" text not null, "extension" text not null, "inserted_at" timestamp(0) not null, "updated_at" timestamp(0) not null);');
    this.addSql('create index "messages_topic_index" on "realtime"."messages" ("topic");');
    ...
    this.addSql('create table "auth"."one_time_tokens" ("id" uuid not null, "user_id" uuid not null, "token_type" one_time_token_type not null, "token_hash" text not null, "relates_to" text not null, "created_at" timestamp(6) not null default now(), "updated_at" timestamp(6) not null default now(), constraint "one_time_tokens_pkey" primary key ("id"), constraint one_time_tokens_token_hash_check check (char_length(token_hash) > 0));');
    ...
    this.addSql('drop table if exists "app"."tweet" cascade;');
